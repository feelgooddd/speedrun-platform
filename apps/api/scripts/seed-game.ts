// prisma/seed-game.ts
// Run with: npx ts-node prisma/seed-game.ts <game-slug> <file-prefix>
// Example: npx ts-node prisma/seed-game.ts hp1gba hp1gba
...
// Platform ID to name mapping from speedrun.com
...
async function getOrCreatePlatform(gameId: string, platformId: string) {
  const platformName = PLATFORM_MAP[platformId] || platformId;
  const slug = platformName.toLowerCase().replace(/\s+/g, '-');
  
  let platform = await prisma.platform.findFirst({
    where: { game_id: gameId, slug }
  });
...
async function seedFile(filename: string, targetGameSlug: string) {
  console.log(`\n${'='.repeat(60)}`);
  ...
  console.log(`Game: ${srcData.game.name}`);
  console.log(`Category: ${categoryName}`);
  console.log(`Total runs: ${srcData.runs.length}\n`);

  // Create or find game using target slug
  let game = await prisma.game.findUnique({ where: { slug: targetGameSlug } });
  if (!game) {
    game = await prisma.game.create({
      data: {
        slug: targetGameSlug,
        name: srcData.game.name,
      }
    });
  ...
  let category = await prisma.category.findFirst({
    where: { game_id: game.id, slug: categorySlug }
  });
  if (!category) {
  ...
  for (const srcRun of srcData.runs) {
    ...
    try {
      ...
      const platform = await getOrCreatePlatform(game.id, srcRun.run.system.platform);
...
async function main() {
  ...
  if (args.length < 2) {
    console.error('Usage: npx ts-node prisma/seed-game.ts <game-slug> <file-prefix>');
    console.error('Example: npx ts-node prisma/seed-game.ts hp1gba hp1gba');
    process.exit(1);
  }

  const [gameSlug, filePrefix] = args;

  console.log(`\n🎮 Starting ${gameSlug.toUpperCase()} data import from speedrun.com\n`);
  ...
  const files = fs.readdirSync('.').filter(f => 
    f.startsWith(filePrefix) && f.endsWith('.json') && !f.includes('cache')
  );
