...
const SRDC_HP7_1_PC_GAME_ID = "3698v0dl";
const SRDC_HP7_1_DS_GAME_ID = "y65yg7de";
...
const PC_CATEGORIES = [
  { srdcId: "xd1lm8k8", name: "Any%", slug: "any" },
  { srdcId: "wk6vnxod", name: "100%", slug: "100" },
];

const DS_CATEGORIES = [
  { srdcId: "9d8p8n7k", name: "Any%", slug: "any" },
  { srdcId: "824rlogd", name: "100%", slug: "100" },
];
...
async function seedStructure() {
  ...
  for (const cat of PC_CATEGORIES) {
    const category = await prisma.category.upsert({
      where: { id: `hp7-1-pc-${cat.slug}` },
      update: {},
      create: {
        id: `hp7-1-pc-${cat.slug}`,
        platform_id: pcPlatform.id,
        name: cat.name,
        slug: cat.slug,
      },
  ...
  for (const cat of DS_CATEGORIES) {
    const category = await prisma.category.upsert({
      where: { id: `hp7-1-ds-${cat.slug}` },
      update: {},
      create: {
        id: `hp7-1-ds-${cat.slug}`,
        platform_id: dsPlatform.id,
        name: cat.name,
        slug: cat.slug,
      },
...
async function main() {
  console.log("Starting HP7-1 seed...");
